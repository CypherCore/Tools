...
namespace DataExtractor.Framework.ClientReader
{
    public class DBReader
    {
        ...
        public static Dictionary<uint, T> Read<T>(string fileName) where T : new()
        {
            Dictionary<uint, T> storage = new Dictionary<uint, T>();

            using (Stream stream = Program.CascHandler.OpenFile(fileName))
            {
                if (stream == null)
                {
    ...
    class WDC2Row
    {
        ...
        T GetFieldValue<T>(int fieldIndex) where T : struct
        {
        ...
        T[] GetFieldValueArray<T>(int fieldIndex, int arraySize) where T : struct
        {
            var columnMeta = _columnMeta[fieldIndex];
